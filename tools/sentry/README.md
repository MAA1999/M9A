...
| `--period`       | 查询时间范围（默认: `30d`，即 Sentry 可靠聚合窗口的上限；超过 30 天只返回截断样本，绝对计数不可用） | `--period 14d`      |
| `--no-fresh`     | 复用 Sentry CLI 本地缓存而不拉取最新数据（同一查询约快 3 倍，适合重复试跑）                         | `--no-fresh`        |
...
报告的全部耗时几乎都花在往返 Sentry CLI 上，本地聚合可忽略：

| 报告                      | 典型耗时 | 内部 sentry 调用数 |
| :------------------------ | -------: | -----------------: |
| `task-failure`（默认 7d） |     ~20s |                  8 |
| `task-trend`（默认 30d）  |     ~50s |                 10 |

三个降低耗时的手段：

1. **互不依赖的查询并发执行**。`task-failure` 的四组查询、`task-trend` 的总量与状态分布查询之间没有先后约束，工具会并发发出而不是逐个串行等待（实测同一组查询：并发 29.7s / 串行 101.6s，结果完全一致）。
2. **`--no-fresh` 复用本地缓存**。默认每次查询都带 `--fresh` 拉取最新数据（单次约 2 到 7 秒）；加上 `--no-fresh` 后同一查询约 2 秒，重复试跑整体快 3 到 5 倍，且输出与 fresh 一致（缓存过期后仍会重新拉取）。
3. **不要并发跑多个报告**。Sentry 侧的瓶颈是单次请求延迟，多个报告同时发起会互相拖慢；串行反而更快。工具会持有一个建议性锁，检测到并发报告时向 stderr 提示一次。

> `--sort` 与 `--limit` 只影响展示，不减少查询量：换排序或换行数都要重新全量查询。
> 需要多种视图时，用 `--format json` 取一次全量再本地切片更划算。
...
4. **时间窗口上限（约 30 天）**：
    - `sentry explore` 对超过约 30 天的窗口不再做真实聚合，而是返回一批固定的截断样本（不同 `--period` 拿到完全相同的结果），此时**绝对计数不可用**，比率趋势仍可参考。
    - `--period` 超过 30 天时工具会向 stderr 打印一次警告；需要精确计数时请改用 30 天以内的窗口（含显式区间）。
