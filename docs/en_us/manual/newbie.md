...
| Android Users | M9A publishes an Android client APK (see [Releases](https://github.com/MAA1999/M9A/releases), named like `M9A-<version>-universal.apk`)<br>Requires Android 9 or newer; on first launch grant notifications and the battery whitelist, then authorise via Shizuku or root<br>Alternatively install the generic client from [MaaFramework](https://github.com/MaaXYZ/MaaFramework/) yourself, see [Usage Method](https://github.com/MaaXYZ/MaaFramework/issues/475) and [MAA Documentation](https://maa.plus/docs/en-us/manual/device/android.html) |
