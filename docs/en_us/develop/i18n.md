...
Everything the client renders — task names, option labels, descriptions, presets, resource and controller names — comes from `interface.json` and the files under `tasks/`. Those strings are localised through the MaaFramework Project Interface V2 i18n mechanism.
...
Paths are relative to the directory holding `interface.json`. Each file is a flat map of key to translated text:
...
Any i18n-capable field whose value begins with `$` is treated as a key into those files. The client resolves it against the active language and falls back to displaying the literal string when the key is missing.
...
`label`, `description`, `pattern_msg`, `icon`, `doc`, plus the project-level `title`, `contact`, `license` and `welcome`. Note that `pipeline_override` is game-facing pipeline data and is never translated.
...
- a `label`, `description` or `pattern_msg` still holding hard-coded Chinese
