...
| 备注 |  | M9A 同时支持搭载 Apple Silicon 和 Intel 芯片的 Mac 电脑，但更推荐搭载 Intel 芯片的 Mac 电脑使用 Mac 自带的多系统安装 Windows，并使用 Windows 版 M9A 和模拟器。 |  | 若您希望使用 Android 实体设备，请前往[此页面](https://maa.plus/docs/zh-cn/manual/device/android.html)。由于此方法较为复杂，不推荐入门玩家使用此方法。 |
