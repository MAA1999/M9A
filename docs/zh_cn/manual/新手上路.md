...
>[!IMPORTANT]
>
> 本章节演示时所使用的图形界面（ MFA 任务管理器）为 MFAAvalonia 1.4.1 版本，M9A 版本为 v3.10.0 。如果您使用的版本不同，界面可能会有所差异，但大体配置流程是相似的。

#### 首次启动

首次通过图形界面（MFA）启动 M9A 时，您将看到 MFA 的主界面：

![MFAAvalonia首次启动](https://github.com/MAA1999/M9A/pull/483#issuecomment-2995673053)
...
- **`启动设置`**
  ...
  我们建议您在此栏目中配置 **`软件路径`** **`附加命令`** 这两个选项。 `软件路径` 用于指定模拟器的启动器的路径， `附加命令` 用于指定启动游戏时需要附加的命令行参数。配置此项可以帮助 M9A 更好地识别和启动游戏。
  ...
  ![软件路径文件选择界面](https://github.com/MAA1999/M9A/pull/483#issuecomment-2995851611)
...
- **`资源类型`**
  ...
  截至 M9A 版本 `3.10.0` ，受支持的区服类型有：`官服` `B服` `OPPO服` `国际服（EN）` `国际服（JP）`。错误选择与对应的模拟器中安装的 1999 区服不匹配的资源类型或游玩不受支持的 1999 区服将无法使用 M9A 。
...
- **`连接`**
  ...
  - ![连接](https://github.com/MAA1999/M9A/pull/483#issuecomment-3000339325)
