...
卡片清单（自最左起，标题即入口参数；含主线与活动）：
雷米特杯失窃案 / 绿湖噩梦 / 行至摩卢旁卡 / 洞穴的囚徒 / 复兴！乌卢鲁运动会 /
朔日手记 / …光灿烂※ / 再见，来亚什基 / 孤独之歌 / …城※ / 7号往事 /
忧郁的热带 / …区黎明※ / 地球上最后的夜晚 / 唐人街影话 / （后续含
疯癫与文明 / 1987宇宙组曲 / 复乐园 等，扫描待续；※=标题被屏幕边缘截断待重扫）
...
| 雷米特杯失窃案（映像/历史） | 导航+地图页到达验证通过（双按钮布局） | 无需适配 |
| 行至摩卢旁卡（映像/历史） | 导航+识别验证通过（双按钮，锚点 HIT） | 无需适配 |
| 绿湖噩梦（映像/历史） | 导航通过；正篇按钮在右中部（ROI 已覆盖）。**疑点待查**：海滩章节截图无模式按钮（锚点 MISS）而雪林章节有——疑似点击地图空白触发 UI 收起态，UI 收起时锚点与列表隐藏但关卡编号仍显示 | 待确认 UI 收起触发条件；若属实需为锚点增加备选判定（编号 token 存在且无正篇/开始行动按钮） |
| 洞穴的囚徒（映像/历史，主线卡） | 详情页为「主线正篇」，导航点击两轮误点相邻卡片后修复 | 双帧位置确认 + pre_wait_freezes：滑动动量未停时识别帧与点击画面错位会点中相邻卡片；地图页验证待重跑 |
