...
`label`、`description`、`desc`、`pattern_msg`、`icon`、`doc`，以及项目级的 `title`、`contact`、`license`、`welcome`。注意 `pipeline_override` 属于面向游戏的 pipeline 数据，不参与翻译。pipeline focus、OCR 识别文本等运行时文本的多语言维护独立成篇，见[运行时文本国际化](./runtime-text-i18n.md)。
...
3. 在 `locales/zh_cn.json` 和 `locales/en_us.json` **两个文件**中补上对应条目。
