...
客户端展示的所有内容——任务名称、选项标签、说明、预设、资源与控制器名称——都来自 `interface.json` 和 `tasks/` 下的文件。这些字符串通过 MaaFramework Project Interface V2 的国际化机制进行本地化。
...
路径相对于 `interface.json` 所在目录。每个文件都是「键 → 译文」的扁平映射：
...
任何支持国际化的字段，只要其值以 `$` 开头，就会被当作翻译文件中的键。客户端按当前语言解析该键；找不到时直接显示原始字符串。
...
`label`、`description`、`pattern_msg`、`icon`、`doc`，以及项目级的 `title`、`contact`、`license`、`welcome`。注意 `pipeline_override` 属于面向游戏的 pipeline 数据，不参与翻译。
...
- 语言文件中存在已无人引用的条目
- 两个语言文件的键集合不一致
- `label`、`description` 或 `pattern_msg` 仍然写死了中文
