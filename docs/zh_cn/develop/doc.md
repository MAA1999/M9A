...
`@mdit/plugin-container`（文档站 `@vuepress/plugin-markdown-hint` 内部使用）在解析容器闭合时检查行偏移与容器开标签缩进是否完全匹配。当闭合符 `:::` 前面紧跟列表项（`- xxx` 或 `1. xxx`）时，prettier 会给闭合符写 2 或 3 空格缩进，但 markdown-it 解析器无法识别这种缩进闭合符，会把它当作列表项的文本字面量，导致：

- 容器未正确闭合
- 后续标题（如 `## 下一节`）被吞进容器
- 文本里残留 `:::` 字面量

**约定**：容器闭合符 `:::` **必须**使用 0 缩进（flush-left），无论容器内是否有列表项。如果 prettier 试图给闭合符加缩进，必须用 `<!-- prettier-ignore -->` 阻止格式化。
